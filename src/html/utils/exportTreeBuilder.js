...
(function() {
    ...
    class ExportTreeBuilder {
        ...
        static buildExportTree(board) {
            ...
            // Organize columns by row (filtering out hidden items)
            const rowMap = new Map();

            board.columns.forEach((column, columnIndex) => {
                // Skip columns with parked/deleted tags
                if (this.isHiddenItem(column.title, null)) {
                    return;
                }
        ...
        /**
         * Get clean column title (remove layout tags and hidden tags)
         */
        static getCleanColumnTitle(title) {
            if (!title) return '';
            // Remove layout tags and hidden tags but keep other tags
            return title
                .replace(/#row\d+/gi, '')
                .replace(/#span\d+/gi, '')
                .replace(/#stack\b/gi, '')
