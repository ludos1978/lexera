...
function setupGlobalDragAndDrop() {
    ...
    /**
     * Calculate column drop target from mouse coordinates (synchronous version)
     * Used when RAF-throttled dragover state may be stale at drop time
     * @param {number} mouseX - Client X coordinate
     * @param {number} mouseY - Client Y coordinate
     * @returns {{stack: HTMLElement, beforeColumn: HTMLElement|null}|null}
     */
    function calculateColumnDropTarget(mouseX, mouseY) {
        ...
        // STEP 2: Within that ROW, find STACK by X coordinate
        let foundStack = null;
        ...
        // Handle drop zone stacks (vertical dividers between stacks)
        if (foundStack && isDropZoneStack) {
            // Drop zone stacks have no columns - find the adjacent regular stack
            // Look for the next stack first (insert at start of next stack)
            const nextStack = foundStack.nextElementSibling;
            if (nextStack && nextStack.classList.contains('kanban-column-stack') && !nextStack.classList.contains('column-drop-zone-stack')) {
                const firstColumn = nextStack.querySelector('.kanban-full-height-column');
                // Return next stack with its first column as beforeColumn (insert before first)
                return { stack: nextStack, beforeColumn: firstColumn };
            }
            // Otherwise, find previous stack (insert at end of previous stack)
            ...
            if (prevStack && prevStack.classList.contains('kanban-column-stack') && !prevStack.classList.contains('column-drop-zone-stack')) {
                // Return previous stack with null beforeColumn (append at end)
                return { stack: prevStack, beforeColumn: null };
            }
            // Fallback: return the drop zone stack itself (will create new stack)
            return { stack: foundStack, beforeColumn: null };
        }

        if (!foundStack) return null;

        // STEP 3: Within that STACK, find beforeColumn by Y coordinate
        const columns = foundStack.querySelectorAll(':scope > .kanban-full-height-column');
        if (columns.length === 0) return { stack: foundStack, beforeColumn: null };

        let beforeColumn = null;
        for (const col of columns) {
            // Skip dragged columns if present
    ...
    function handleExternalDrop(e) {
        ...
        // CRITICAL: For parked/deleted column drags, RAF timing means dragState values may be stale
        // ALWAYS calculate drop target synchronously from mouse coordinates for these drags
        let capturedDropTargetStack = null;
        ...
        if (dragState.draggedParkedColumn || dragState.draggedDeletedColumn) {
            ...
            capturedDropTargetBeforeColumn = dragState.dropTargetBeforeColumn;
        }
        ...
        // Priority 2: Check for parked items being restored first
        if (dragState.parkedItemIndex !== undefined && dragState.parkedItemIndex !== null) {
            if (dragState.draggedParkedColumn) {
                // Parked column being restored - pass captured drop target
                restoreParkedColumn(dragState.parkedItemIndex, { x: e.clientX, y: e.clientY }, capturedDropTargetStack, capturedDropTargetBeforeColumn);
                finalizeExternalDragState();
                return;
            } else if (dragState.draggedClipboardCard) {
                // Parked task being restored
                restoreParkedTask(dragState.parkedItemIndex, { x: e.clientX, y: e.clientY });
                finalizeExternalDragState();
                return;
            }
        }

        // Priority 2b: Check for deleted items being restored
        if (dragState.deletedItemIndex !== undefined && dragState.deletedItemIndex !== null) {
            if (dragState.draggedDeletedColumn) {
                // Deleted column being restored - pass captured drop target
                restoreDeletedColumn(dragState.deletedItemIndex, { x: e.clientX, y: e.clientY }, capturedDropTargetStack, capturedDropTargetBeforeColumn);
                finalizeExternalDragState();
                return;
            } else if (dragState.draggedClipboardCard) {
                // Deleted task being restored
                restoreDeletedTask(dragState.deletedItemIndex, { x: e.clientX, y: e.clientY });
                finalizeExternalDragState();
                return;
            }
        }

        // Check dragState for text clipboard/empty cards (non-parked, non-deleted)
        if (dragState.draggedClipboardCard) {
        ...
        if (textData2) {
            if (textData2.startsWith('PARKED_ITEM:')) {
                // Handle parked item being restored to board
                const parkedData = textData2.substring('PARKED_ITEM:'.length);
                handleParkedItemDrop(e, parkedData);
            } else if (textData2.startsWith('DELETED_ITEM:')) {
                // Handle deleted item being restored to board
                const deletedData = textData2.substring('DELETED_ITEM:'.length);
                handleDeletedItemDrop(e, deletedData);
            } else if (textData2.startsWith('CLIPBOARD_CARD:')) {
    ...
    document.addEventListener('drop', function(e) {
        // Check if drop is on a valid drop target (board, park, or trash)
        const isInBoard = boardContainer.contains(e.target);
        const isOnParkTarget = e.target.closest('#clipboard-drop-target') || e.target.closest('#clipboard-drop-menu');
        const isOnTrashTarget = e.target.closest('#trash-drop-target') || e.target.closest('#trash-drop-menu');

        if (!isInBoard && !isOnParkTarget && !isOnTrashTarget) {
            e.preventDefault();
            // Clean up indicators if drop happens outside board/park/trash
            cleanupExternalDropIndicators();
        }
    }, false);
    ...
    // Expose shared functions on window for use by global functions (restoreParkedColumn, etc.)
    window.syncColumnDataToDOMOrder = syncColumnDataToDOMOrder;
    window.finalizeColumnDrop = finalizeColumnDrop;
    ...
    document.addEventListener('mouseup', function(e) {
        ...
        if (dragState.isDragging) {
            // Skip reset if mouseup is on a valid drop target (Park, Trash, or board)
            // The drop event will handle cleanup in those cases
            const isOnParkTarget = e.target.closest('#clipboard-drop-target') || e.target.closest('#clipboard-drop-menu');
            const isOnTrashTarget = e.target.closest('#trash-drop-target') || e.target.closest('#trash-drop-menu');
            const isOnBoard = boardContainer.contains(e.target);

            if (isOnParkTarget || isOnTrashTarget || isOnBoard) {
                // Let the drop handler manage the state cleanup
                return;
            }
    ...
    // Export cleanup functions for use by parking system
...
/**
 * Handle drop of a parked item back onto the board
 */
function handleParkedItemDrop(e, parkedData) {
    try {
        const parsed = JSON.parse(parkedData);
        const parkedIndex = parsed.index;
        ...
        } else if (parkedType === 'column') {
            if (typeof restoreParkedColumn === 'function') {
                restoreParkedColumn(parkedIndex, dropPosition);
            }
        }
    } catch (error) {
        console.error('Failed to handle parked item drop:', error);
    }
}
...
function setupColumnDragAndDrop() {
    ...
    document.addEventListener('dragover', e => {
        ...
        // Parked/deleted columns use EXTERNAL state but should get column-like highlighting
        const isParkedColumnDrag = dragMode === dragDropStateMachine.states.EXTERNAL &&
        ...
        // For parked column drags: Store mouse position SYNCHRONOUSLY so drop can calculate target
        // (RAF timing means state may not be updated before drop fires)
        if (isParkedColumnDrag) {
            dragState.lastParkedColumnDragX = mouseX;
            dragState.lastParkedColumnDragY = mouseY;
        }
        ...
        requestAnimationFrame(() => {
            ...
            const stillParkedColumnDrag = currentMode === dragDropStateMachine.states.EXTERNAL &&
                (dragState.draggedParkedColumn || dragState.draggedDeletedColumn);
            if (!stillColumnDrag && !stillTemplateDrag && !stillTaskDrag && !stillClipboardDrag && !stillEmptyCardDrag && !stillParkedColumnDrag) {
                return;
            }
            ...
            // Handle drop zone stacks (vertical dividers between stacks) - only for column/template/parked-column drags
            // Task/clipboard/emptycard drags skip drop zones - they go INTO columns
            if (foundStack && isDropZoneStack && (stillColumnDrag || stillTemplateDrag || stillParkedColumnDrag)) {
...
// ============================================================================
// INTERNAL CLIPBOARD PARKING SYSTEM
// ============================================================================
 ...
 */
function initializeParkedItems() {
    parkedItems = [];
...
function updateParkedItemsUI() {
    ...
    if (parkedItems.length > 0) {
        countElement.textContent = parkedItems.length;
    ...
    if (parkedItems.length === 0) {
        ...
        parkedItems.forEach((item, index) => {
            ...
            itemElement.className = 'clipboard-drop-item';
            itemElement.draggable = true;
            itemElement.dataset.parkedIndex = index;
            itemElement.dataset.parkedType = item.type;
            itemElement.dataset.parkedId = item.id;
            ...
            itemElement.addEventListener('dragend', handleParkedItemDragEnd);
...
/**
 * Handle drag over the clipboard drop target
 */
function handleClipboardDropTargetDragOver(event) {
...
/**
 * Handle drag leave from the clipboard drop target
 */
function handleClipboardDropTargetDragLeave(event) {
    const target = document.getElementById('clipboard-drop-target');
    if (target) {
...
/**
 * Handle drop on the clipboard drop target (park item)
 */
function handleClipboardDropTargetDrop(event) {
    event.preventDefault();
    event.stopPropagation();
...
/**
 * Handle drag start for a parked item (dragging back to board)
 */
function handleParkedItemDragStart(event, index) {
    const item = parkedItems[index];
    if (!item) return;

    event.dataTransfer.effectAllowed = 'move';
    event.dataTransfer.setData('text/plain', 'PARKED_ITEM:' + JSON.stringify({
        index: index,
        type: item.type,
        id: item.id
    }));
    ...
    if (item.type === 'task') {
        ...
        dragState.draggedParkedColumn = item;
        dragDropStateMachine.start(dragDropStateMachine.states.EXTERNAL, {
            source: 'parked-column',
            parkedIndex: index
        });
    }

    dragState.isDragging = true;
    dragState.parkedItemIndex = index;

    // Visual feedback
    event.target.classList.add('dragging');

    // Keep dropdown open during drag
    const dropMenu = document.getElementById('clipboard-drop-menu');
    if (dropMenu) {
...
/**
 * Handle drag end for a parked item
 */
function handleParkedItemDragEnd(event) {
    ...
    if (dragState.leftView) {
        // Item was dragged outside - keep it parked
        dragState.leftView = false;
    }

    // Clean up
    dragState.draggedClipboardCard = null;
    dragState.draggedParkedColumn = null;
    dragState.parkedItemIndex = null;
    dragState.isDragging = false;
    dragDropStateMachine.reset('parked-item-dragend');
}
    ...
    if (dropPosition) {
        ...
        if (dropResult && dropResult.columnId) {
            ...
            if (sourceColumn && targetColumn && sourceTaskIndex >= 0) {
                ...
                let insertIndex = dropResult.insertionIndex;
...
/**
 * Restore a parked column to the board at the drop position.
 * Uses the same approach as insertColumnAtPosition/processColumnDrop:
 * 1. Remove tag and render (column appears)
 * 2. Move DOM element to target position
 * 3. syncColumnDataToDOMOrder() + finalizeColumnDrop()
 *
 * @param {number} parkedIndex - Index in parkedItems array
 * @param {Object} dropPosition - {x, y} mouse coordinates
 * @param {Element|null} capturedDropTargetStack - Drop target stack captured before cleanup
 * @param {Element|null} capturedDropTargetBeforeColumn - Before column captured before cleanup
 */
function restoreParkedColumn(parkedIndex, dropPosition, capturedDropTargetStack, capturedDropTargetBeforeColumn) {
    ...
    // Render board - column will appear in its data position
    if (typeof window.renderBoard === 'function') {
        window.renderBoard();
    }
...
/**
 * SHARED: Move a column to the drop target position after render.
 * Reuses the same logic as processColumnDrop and insertColumnAtPosition:
 * 1. Find column element after render
 * 2. Move to target stack/position
 * 3. Sync data to DOM order
 * 4. Finalize (normalize tags, update UI)
 *
 * @param {string} columnId - The column to move
 * @param {string|null} beforeColumnId - Column to insert before (or null for end)
 * @param {string|null} targetStackFirstColId - ID of first column in target stack (for finding stack after render)
 * @param {Object} options - Optional settings
 * @param {boolean} options.preserveStackState - If true, don't normalize stack tags (for restored columns)
 */
function moveColumnToDropTarget(columnId, beforeColumnId, targetStackFirstColId, options = {}) {
    ...
    if (targetStack || beforeColumn) {
        const sourceStack = columnElement.closest('.kanban-column-stack');
        const actualTargetStack = targetStack || beforeColumn?.closest('.kanban-column-stack');

        if (actualTargetStack) {
            if (beforeColumn) {
                actualTargetStack.insertBefore(columnElement, beforeColumn);
            } else {
                actualTargetStack.appendChild(columnElement);
            }

            // Clean up empty source stack
            if (sourceStack && sourceStack !== actualTargetStack && !sourceStack.querySelector('.kanban-full-height-column')) {
                sourceStack.remove();
            }
        }
    }

    // Sync data to DOM order
...
// Make clipboard parking functions globally available
window.handleClipboardDropTargetDragOver = handleClipboardDropTargetDragOver;
window.handleClipboardDropTargetDragLeave = handleClipboardDropTargetDragLeave;
window.handleClipboardDropTargetDrop = handleClipboardDropTargetDrop;
window.initializeParkedItems = initializeParkedItems;
window.updateParkedItemsUI = updateParkedItemsUI;
window.parkTask = parkTask;
window.parkColumn = parkColumn;
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', attachParkEventListeners);
} else {
    attachParkEventListeners();
}

// ============================================================================
// INTERNAL TRASH/DELETION SYSTEM
// ============================================================================
// Allows users to "delete" tasks and columns by tagging them with #hidden-internal-deleted.
// Deleted items are hidden from view but stored in the board data.
// Users can restore them from the trash dropdown or permanently remove them.
...
function updateDeletedItemsUI() {
    ...
    if (deletedItems.length > 0) {
        countElement.textContent = deletedItems.length;
        countElement.classList.add('has-items');
    } else {
        countElement.textContent = '';
        countElement.classList.remove('has-items');
    ...
    const existingFooter = dropdown?.querySelector('.trash-drop-footer');
    if (existingFooter) {
    ...
    if (deletedItems.length === 0) {
        ...
        deletedItems.forEach((item, index) => {
            const itemElement = document.createElement('div');
            itemElement.className = 'trash-drop-item';
            itemElement.draggable = true;
            itemElement.dataset.deletedIndex = index;
            itemElement.dataset.deletedType = item.type;
            itemElement.dataset.deletedId = item.id;
            ...
            itemElement.addEventListener('dragend', handleDeletedItemDragEnd);

            itemsContainer.appendChild(itemElement);
        });

        // Add "Empty Trash" button footer
        if (dropdown) {
 ...
 */
function handleTrashDropTargetDragOver(event) {
    event.preventDefault();
    event.stopPropagation();
    ...
    if (isValidDrop) {
        ...
        pendingTrashDrop = {
            ...
            parkedItemIndex: dragState.parkedItemIndex
        };
    }
}

/**
 * Handle drag leave from the trash drop target
 */
function handleTrashDropTargetDragLeave(event) {
    const target = document.getElementById('trash-drop-target');
    if (target) {
        target.classList.remove('drag-over');
    }
...
/**
 * Handle drop on the trash drop target (delete item)
 */
function handleTrashDropTargetDrop(event) {
    event.preventDefault();
    event.stopPropagation();

    // Use captured state from dragover if live state was cleared by mouseup
    const taskToUse = dragState.draggedTask || pendingTrashDrop?.draggedTask;
    const columnToUse = dragState.draggedColumn || pendingTrashDrop?.draggedColumn;
    const clipboardCardToUse = dragState.draggedClipboardCard || pendingTrashDrop?.draggedClipboardCard;
    ...
    if (taskToUse) {
        ...
        trashColumn(columnToUse);
    } else if (clipboardCardToUse && parkedIndexToUse !== undefined) {
        // Parked task being moved to trash
...
/**
 * Handle drag start for a deleted item (dragging back to board)
 */
function handleDeletedItemDragStart(event, index) {
    const item = deletedItems[index];
    if (!item) return;

    event.dataTransfer.effectAllowed = 'move';
    event.dataTransfer.setData('text/plain', 'DELETED_ITEM:' + JSON.stringify({
        index: index,
        type: item.type,
        id: item.id
    }));
    ...
    if (item.type === 'task') {
        ...
        // For columns, we need special handling
        dragState.draggedDeletedColumn = item;
        dragDropStateMachine.start(dragDropStateMachine.states.EXTERNAL, {
            source: 'deleted-column',
            deletedIndex: index
        });
    }

    dragState.isDragging = true;
    dragState.deletedItemIndex = index;

    // Visual feedback
    event.target.classList.add('dragging');

    // Keep dropdown open during drag
    const dropMenu = document.getElementById('trash-drop-menu');
    if (dropMenu) {
        dropMenu.classList.add('dragging-from');
    }
}
...
/**
 * Handle drag end for a deleted item
 */
function handleDeletedItemDragEnd(event) {
    event.target.classList.remove('dragging');
    ...
    // Check if the item was dropped successfully
    // If leftView flag is set, keep the item in trash
    if (dragState.leftView) {
        dragState.leftView = false;
    }
    ...
    // Clean up
    dragState.draggedClipboardCard = null;
    dragState.draggedDeletedColumn = null;
    dragState.deletedItemIndex = null;
    dragState.isDragging = false;
    dragDropStateMachine.reset('deleted-item-dragend');
}
 ...
 * Uses the same approach as restoreParkedColumn/insertColumnAtPosition.
 *
 * @param {number} deletedIndex - Index in deletedItems array
 * @param {Object} dropPosition - {x, y} mouse coordinates
 * @param {Element|null} capturedDropTargetStack - Drop target stack captured before cleanup
 * @param {Element|null} capturedDropTargetBeforeColumn - Before column captured before cleanup
 */
function restoreDeletedColumn(deletedIndex, dropPosition, capturedDropTargetStack, capturedDropTargetBeforeColumn) {
    const item = deletedItems[deletedIndex];
    if (!item || item.type !== 'column') return;
...
/**
 * Restore a deleted item by clicking the restore button (to original location or first column)
 */
function restoreDeletedItemByIndex(index) {
...
/**
 * Handle drop of deleted item on the board
 */
function handleDeletedItemDrop(e, dataString) {
    const data = JSON.parse(dataString);
    const deletedIndex = data.index;

    if (deletedIndex === undefined || deletedIndex < 0 || deletedIndex >= deletedItems.length) {
        return;
    }
