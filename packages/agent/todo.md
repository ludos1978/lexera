...
- [ ] redo an interation of font size and style unification. there must only be these styles:
  ...
  - uppercase text
  - text with background color where the text must be in a good contrast to the chosen background. take the values formulae from version 1, also add contrast enhancing bloom around it's background exactly as in version 1!

- [ ] remove the dropdown in "workspaces" where "all workspaces" or the individual ones can be chosen. it's functionality should have been moved down to the boards below

- [x] a row, stack, column should only ever show "+ add (type of child)" if there is none in the element. implement it with css alone, not javascript or anything else!
  - `packages/lexera-kanban/src/app.js` now renders the row/stack/column add affordances unconditionally; visibility is no longer gated by JS emptiness branches.
  - `packages/lexera-kanban/src/app.css` now hides those affordances via structural `:has(...)` selectors when rows/stacks/columns/cards are present, while keeping the inline add-card composer visible.
  - Regression coverage lives in `packages/lexera-kanban/tests/cssOnlyAddAffordances.test.js`.

- [x] dragging an empty excalidraw into the board from the "create row, stack, column, card" isnt adding a empty exclidraw that the user then can edit! the same must work with all other types as well that we support (currently excalidraw, draw.io, maybe later also docx, xlsx, pptx etc.) . its not even logging anything at all!
  - `packages/lexera-kanban/src/menu/rowStackMenu.js` now routes built-in file-backed templates through a generic `insertTextContentForEntity(...)` seam instead of treating them as card-only. That means Excalidraw / Draw.io embeds can now create a row, stack, column, or card depending on the drop target.
  - `packages/lexera-kanban/src/hiddenItems/hiddenItemsDropdown.js` and `packages/lexera-kanban/src/app.js` now pass header-source drag/drop through that same insertion seam, so click and drag creation use the same content path.
  - Regression coverage lives in `packages/lexera-kanban/tests/headerCreationActions.test.js` and `packages/lexera-kanban/tests/headerSourceInsertionInvariants.test.js`.

- [x] the burger menu in the workspace has some elements that are not readable, maybe the font has some broken colors or whatever. fix this!
  - The workspace-shell tab overflow menu in `packages/lexera-kanban/src/workspace/workspaceShell.css` now uses the shared menu surface/item contrast tokens (`--menu-bg`, `--menu-border`, `--menu-item-fg`, `--menu-item-fg-hover`) instead of weaker tab-strip colors, and the close buttons no longer fade to near-invisible contrast.
  - Regression coverage lives in `packages/lexera-kanban/tests/workspaceShellOverflowMenuStyles.test.js`.

- [x] backend settings has network and configuration, but configuration is empty. remove the configuration tab (leave all functions intact)!
  - Shared management presets in `packages/lexera-shared/management.js`, `packages/lexera-kanban/src/management.js`, and `packages/lexera-backend/src/management.js` now expose only `network`/`logs` for backend settings; legacy `config` section routing is kept as a compatibility alias that resolves to `network`.
  - Regression coverage lives in `packages/lexera-kanban/tests/management.test.js`.
...
- [ ] there must be one update path if elements are changed in the structure of the kanban/canvas, and another one if content is changed within the boards. 
  ...
  - The load-path/runtime bypasses on `fullBoardData` and `activeBoardData` are fixed, but the app still has overlapping state owners. `packages/lexera-kanban/src/app.js`, `packages/lexera-kanban/src/board/boardList.js`, and `packages/lexera-kanban/src/board/orderHelpers.js` still coordinate through a mix of local vars, `LexeraRuntime`, hierarchy projection cache, and iframe bridge payloads rather than one authoritative board-session store.
  ...
  - Sidebar/tree identity is only partially migrated. Focus/highlight now prefers stable row/stack/column/card ids, but `packages/lexera-kanban/src/dragdrop/dndListeners.js`, `packages/lexera-kanban/src/dragdrop/dragDropHandlers.js`, and several mutation/action contexts still rely on `data-col-index` / `data-card-index` and row/stack display indices, so reorder and cross-view move behavior is not fully id-based yet.
  - Frontend legacy-column conversion is partially unified now: `packages/lexera-kanban/src/board/boardList.js::rowsFromLegacyColumns()` delegates to the shared frontend converter in `packages/lexera-kanban/src/board/orderHelpers.js::buildRowsFromLegacyColumns()`. The remaining divergence is export/shared/backend: `packages/lexera-kanban/src/export/exportTreeBuilder.js`, `packages/shared/src/markdownParser.ts`, and `packages/lexera-core/src/parser.rs` still do not share one canonical legacy-import boundary.
  ...
  - Missing regression coverage is now narrower but still real: no focused test protects hidden-count correctness, row/stack layout-tag stripping, or stable hierarchy identity/highlighting after reorder.
...
- [ ] **Single-source cleanup pass** — execute the duplicate functions, duplicate data structures, duplicate persisted data, and shared asset consolidation backlog in `packages/todo.md` so each data class has one authoritative home.
  ...
  - Legacy board normalization is still not repo-wide single-source. `boardList.js` now delegates to `orderHelpers.js`, but `exportTreeBuilder.js`, `packages/shared/src/markdownParser.ts`, and `lexera-core/src/parser.rs` still normalize legacy boards independently.
