...
describe('buildSidebarTreeNodes', () => {
  it('keeps row and stack nodes even for single-row single-stack boards', () => {
    ...
    const nodes = U.buildSidebarTreeNodes(rows, 'board-1', { rows: [], stacks: [], columns: [] }, false, true, true);
  ...
  it('keeps every row as a top-level node when multiple rows exist', () => {
    ...
    const nodes = U.buildSidebarTreeNodes(rows, 'board-1', { rows: [], stacks: [], columns: [] }, false, false, false);
