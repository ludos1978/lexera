...
const LexeraDashboard = (function () {
  ...
  function normalizeBoardLayoutValue(value) {
    return getCanvasModeHelpers().normalizeBoardLayoutValue(value);
  }

  function normalizeCanvasGridValue(value) {
    return getCanvasModeHelpers().normalizeCanvasGridValue(value);
  }
  ...
  function buildRowsFromLegacyColumns(cols, fallbackTitle) {
    ...
    for (var r = 0; r < rowNumbers.length; r++) {
      var rowNumber = rowNumbers[r];
      var entries = rowsByNumber[rowNumber] || [];
      var groups = [];
      ...
      rows.push({
        id: 'row-' + Date.now() + '-' + r,
        title: multipleRows ? ('Row ' + rowNumber) : (fallbackTitle || 'Default'),
        stacks: stacks
      });
    }
    return rows;
  ...
  function applySidebarSectionLayout() {
    if (SidebarResize) SidebarResize.applySidebarSectionLayout();
  }
  ...
  function renderDashboardPinnedList() {
    ...
    if (!dashboardState.pinnedQueries || dashboardState.pinnedQueries.length === 0) {
      setDashboardGroupEmptyState(getElDashboardPinnedList(), true);
  ...
  function init() {
    ...
    if (SidebarResize) {
      ...
      SidebarResize.setupSidebarSectionResize();
      SidebarResize.setupSidebarWidthResize();
    }
    setupWorkspaceShell();
