...
### Modified: `src/html/dragDrop.js`
- `notifyBoardUpdate(affectedColumnIds?)` — (MODIFIED) Accepts optional array of column IDs. Calls `syncAndRenderSiblings` for affected columns, then `markUnsavedChanges`, sends `boardUpdate`, and re-renders directly affected columns. Card-level callers (trashTask, parkTask, archiveTask) pass their column ID; column-level and multi-column callers omit it for full render.
...
- `syncSharedIncludeColumns(sourceColumnId)` — (DATA ONLY) Deep-copies all cards from the source column to every sibling column sharing the same include file (using normalized path comparison), regenerating column-specific IDs (`task-${columnId}-${index}`). Returns array of additionally affected column IDs. Exposed globally as `window.syncSharedIncludeColumns`. Supersedes the removed `propagateInternalTagToSharedIncludes`.
- `syncAndRenderSiblings(columnIds)` — Consolidated sync+render helper. Calls `syncSharedIncludeColumns` for each column ID, then `renderBoard({ columns: siblings })` for any affected siblings. Exposed globally as `window.syncAndRenderSiblings`. Called from: `notifyBoardUpdate`, `sendBoardUpdateToBackend`, `processTaskDrop`, `createTasksWithContent`, `_saveTaskField` (cardEditor.js), `updateColumnContent` handler (webview.js), `updateCardContent` handler (webview.js).
- `sendBoardUpdateToBackend(sourceColumnIds)` — Calls `syncAndRenderSiblings`, then `markUnsavedChanges` and sends `boardUpdate` to backend. Used by restore functions that don't go through `notifyBoardUpdate`.
