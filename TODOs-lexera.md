...
- [ ] (in progress) ~~View > Developer Tools (All Views) does nothing.~~ → ~~entire native dropdown menu does nothing~~ → only ONE inspector opens, not all. **Sequence of fixes (2026-05-05):**
  ...
  - **Remaining:** the menu now fires correctly but `Developer Tools (All Views)` only opens ONE inspector instead of N. Diagnostic `eprintln!` lines added to [commands.rs open_devtools_all](lexera-kanban/src-tauri/src/commands.rs#L695) (uncommitted) print webview count + per-label opened/skipped status when the user runs the menu. Awaiting that output to determine which of three causes is actual: (a) `app.webviews()` returns only 1, (b) `is_devtools_open()` lies after first call, (c) `wv.open_devtools()` no-ops on child webviews on macOS.
