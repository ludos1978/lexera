...
- [ ] (in progress) **Verify Isolation:** Create a verification script or automated test that spawns two top-level windows and confirms that switching workspaces or opening boards in one does not affect the other. (Approach: consolidated source-level contract — Vitest can't drive two real Tauri windows, but a single test file that audits all the multi-window invariants prevents regression and documents the architecture.)
