...
- [ ] **bottom-dock fold asymmetry** — `foldPane` (workspaceShell.js:683-712) treats left/right via `collapseDock` (sets `dockSizes[dock]=0`) but bottom + center splits via `state.foldedPanes[nodeId]=ratio` + `parent.ratio=0|1`. Two state shapes for the same UX, two render paths, two unfold paths. Unify the bottom-dock case to also call `collapseDock` so it shares the same dockSizes-driven fold strip the side docks use. Estimated 60-80 lines removed. **DEFERRED until cecd3aa7 is verified** — don't stack changes onto an unverified fix.
