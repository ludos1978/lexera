...
- [ ] (in progress) **2.2** Wire `reconciler.reconcile(state)` as the first call in `render()` (workspaceShell.js:~3540). Skip while `state.dragTabId` is set. Phase 1 individual fixes stay in place as belt-and-braces.
