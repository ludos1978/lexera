...
- [ ] (input required) ~~View > Developer Tools (All Views) does nothing.~~ **Two-stage fix.** Stage 1 (commit 60f68834): added handlers in shell `EXACT_ACTIONS` so the action invokes Tauri directly instead of forwarding to the (nonexistent) active frame. **Stage 2 (commit 26bdcaf1):** the stage-1 commit registered handlers under fabricated ids `open-all-inspectors` / `toggle-inspector` that the Tauri menu never emits — the real ids from [app_menu.rs:161-162](lexera-kanban/src-tauri/src/app_menu.rs#L161) are `view-inspector` (F12) and `view-inspector-all` (Cmd+Alt+Shift+I). With the mismatched names the action still fell through and the user saw nothing. Renamed the EXACT_ACTIONS keys to match. New contract test reads app_menu.rs directly so any future menu-id rename breaks the test instead of the user. **Awaiting real-app confirmation:** click View > Developer Tools (All Views) — expect multiple DevTools windows to open and an `[ws-shell] open_devtools_all → opened N view(s)` entry in the log panel.
...
- [ ] (input required) shell IIFE was booting twice in the main window on `cargo tauri dev`, spawning two complete sets of child Tauri webviews at identical screen coordinates ("windows visible multiple times" / "content within the background view"). Root cause: `lexera-shared/scripts/sync-runtime-assets.mjs` ran via `beforeDevCommand` and unconditionally `copyFileSync`-ed six files into `lexera-kanban/src/` (= `frontendDist`). Identical-content writes still bumped destination mtime, the Tauri 2 frontend watcher reloaded the main webview a few hundred ms after boot, and the old shell's `destroyAll()` fire-and-forget IPCs lost the race against the new shell's spawn loop. **Fix: make sync-runtime-assets idempotent — read both source and destination, compare bytes, write only on mismatch.** Same CLI contract, no Rust/Tauri changes. Pinned by `syncRuntimeAssetsIdempotent.test.js` (5 assertions: first-run writes 6, second-run writes 0, mtimes preserved on no-change, mutating one dest causes exactly that one to be rewritten). (commit cecd3aa7) **Awaiting real-app verification:** relaunch `cargo tauri dev`, confirm only ONE `[ws-debug] #1 BOOT shell` log line and only ONE set of `multiview_spawn` entries in stdout.

- [ ] (input required) ~~**bottom-dock fold asymmetry** — `foldPane` (workspaceShell.js:683-712) treats left/right via `collapseDock` (sets `dockSizes[dock]=0`) but bottom + center splits via `state.foldedPanes[nodeId]=ratio` + `parent.ratio=0|1`. Two state shapes for the same UX, two render paths, two unfold paths. Unify the bottom-dock case to also call `collapseDock` so it shares the same dockSizes-driven fold strip the side docks use. Estimated 60-80 lines removed.~~ Shipped: `foldPane` now routes ALL side docks (left, right, bottom) through `collapseDock`; `state.foldedPanes` is reserved for center splits only. Removed the cec379d5 view-header injection (no longer reachable). Net 23 lines deleted in 0635f335. Pinned by a new test in workspaceShellPanelDockLifecycle.test.js: `foldPane(nodeId)` on a side-dock tabset sets `dockSizes[dock]=0` and never adds anything to `foldedPanes`. (commit 0635f335 source + bea7cd54 test). **Awaiting real-app confirmation that the bottom-dock fold strip is now visible with the Connected/N logs/N users/N pending badges, behaving the same as the left/right strips.**

- [ ] **synchronous destroy-all on `beforeunload`** (multiviewWebview.js:1158-1167) — currently fire-and-forget per-tab `destroy()` IPCs that race a reload. Replace with a single `multiview_destroy_all_for_window` IPC that the Rust side handles atomically. Defensive: even if a future change re-introduces a reload trigger, no orphan webviews can survive into the next boot.
...
- [ ] **webview-mgr label-collision protection in Rust** — `is_reserved_window_label` (`webview_mgr.rs:25-41`) only blocks top-level WINDOW labels, never sibling child webview labels. Backstop: add a check that rejects `multiview_spawn` when the parent already hosts a sibling at the same coordinates with a different label.
