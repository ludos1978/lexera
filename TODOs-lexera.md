...
- [x] ~~Workspace dropdown removed — `renderWorkspaceSelect` in app.js (2 refs)~~ — outdated note: no `<select>` workspace widget exists in the markup. `renderWorkspaceSelect()` is now a 3-line mirror-refresh helper at [boardList.js:1597](lexera-kanban/src/board/boardList.js#L1597) (`resolveActiveWorkspaceId(null); syncMirroredWorkspaceViews();`) — vestigial name, not a UI widget. Renaming the helper to `refreshWorkspaceMirrors` is safe future polish but doesn't keep the dropdown around.
