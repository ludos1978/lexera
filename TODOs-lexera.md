...
- [ ] (in progress) **Fix Event Scoping in `embedMenu.js`:** Update `LexeraEmbedMenu.tauriListen` to use the current webview's scope instead of `{ kind: 'Any' }`. This will prevent events like `menu-action` from leaking across windows.
