...
- [x] ~~**Verify Isolation:** Create a verification script or automated test that spawns two top-level windows and confirms that switching workspaces or opening boards in one does not affect the other.~~ — 5b944b13 (`multiWindowIsolationContract.test.js` — 24 tests across 6 architectural patterns: per-window Rust state shape, dynamic window resolution in lifecycle commands, scoped emit for window-scoped events, bootId-embedded webview labels, full close-cleanup, per-window UX state via WINDOW_DEFS. Vitest can't drive two real Tauri windows, so this is source-level verification — every concrete fix has its own per-feature contract test that covers call-site detail.)

### Unsorted (leave this header here!)

- [x] ~~when closing windows, it should close the view, but not the application!~~ — bf885953 (every window closes on red-X click — main no longer special-cased to minimise. `RunEvent::ExitRequested` is intercepted; `USER_REQUESTED_QUIT` atomic flag is set only by `quit_app` (Cmd+Q / File > Quit). Closing the last window keeps the process alive — re-openable via the macOS menu bar. 5 contract tests pin the new shape.)
