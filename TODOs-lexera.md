...
- [ ] (input received 2026-05-05) ~~**bottom-dock fold asymmetry** — `foldPane` (workspaceShell.js:683-712) treats left/right via `collapseDock` (sets `dockSizes[dock]=0`) but bottom + center splits via `state.foldedPanes[nodeId]=ratio` + `parent.ratio=0|1`. Two state shapes for the same UX, two render paths, two unfold paths. Unify the bottom-dock case to also call `collapseDock` so it shares the same dockSizes-driven fold strip the side docks use. Estimated 60-80 lines removed.~~ Shipped: `foldPane` now routes ALL side docks (left, right, bottom) through `collapseDock`; `state.foldedPanes` is reserved for center splits only. Removed the cec379d5 view-header injection (no longer reachable). Net 23 lines deleted in 0635f335. Pinned by a new test in workspaceShellPanelDockLifecycle.test.js. (commit 0635f335 + bea7cd54). **Status update from user 2026-05-05:** badges still not visible. User insight: in folded state the log viewport is 0×0 (so its own DOM can't paint anything, including its header). In the OLD design the header came from a unified system (the shell rendered it for the panel). In the NEW design the panel header is owned by the *background webview* — the shell — but the SHELL no longer injects status badges into that header for the log panel since the cec379d5 → 0635f335 unification. **Next step:** the fold-strip rendered by the shell must read the live log status (Connected / N logs / N users / N pending) — likely via the same `log-snapshot` channel the log subapp consumes — and inject the badges into the bottom-dock view header *before* foldPane collapses the panel to 0×0. Current code path: `renderFoldStrip` for left/right; for bottom-dock the `ws-view-header` element from `_test_findHeaderForBottomLogPanel` is the injection target. Owner question for the user: should the badges live on the shell's fold-strip permanently (visible whether folded or not), or only render when bottom-dock dockSize === 0? Default to "always-on header strip; bigger when expanded" matches the side-dock parity goal.
