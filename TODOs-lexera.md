...
- [ ] (input required) ~~View > Developer Tools (All Views) does nothing.~~ Root cause: shell's `handleBoardAction` had no case for `open-all-inspectors` / `toggle-inspector`, so the action fell through to `forwardActionToActiveFrame` which silently dropped it on panel-only layouts and added needless round-trips otherwise. Fix: handle inspector actions in `EXACT_ACTIONS` and call `window.__TAURI__.core.invoke('open_devtools_all'|'toggle_devtools')` directly from the shell. Both paths log via `lexeraLog` so the in-app log shows the action firing + Rust's return value. 3 new vitest cases in workspaceShellPanelDockLifecycle.test.js. (commit 60f68834) **Awaiting real-app confirmation: click View > Developer Tools (All Views) — expect N DevTools windows to open and an `[ws-shell] open_devtools_all → opened N view(s)` entry in the log panel.**
