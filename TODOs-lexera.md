...
- [ ] (in progress) the workspace must only open ONE workspace, not all at once. Hovering "Open Workspace" should reveal a submenu of all configured workspaces, not a popup dialog. The current dialog also crashes with "No workspaces available" because the action handler reads `d.workspaces` but app.js never wired that into the action dep bag. Plan: (a) wire `workspaces` into the bag so existing dialog populates, (b) replace dialog with a native Tauri dynamic submenu rebuilt on every workspace catalog change, (c) tear out `ALL_WORKSPACES_ID` so each window owns exactly one workspace for its lifetime.
