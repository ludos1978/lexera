...
- [x] ~~the workspace must only open ONE workspace, not all at once. Hovering "Open Workspace" should reveal a submenu of all configured workspaces.~~ — 93c81a22 + 83dff022 + 96758e21 ((a) action dep bag now exposes `getWorkspaces` (live-getter) so the chooser fallback no longer reports "No workspaces available". (b) File menu now hosts a NATIVE dynamic submenu `Open Workspace ▶` with one entry per workspace, rebuilt every catalog change via the new `set_workspaces_submenu` Tauri command; each entry dispatches `open-workspace:<id>` straight to `WorkspaceShell.openWorkspaceWindow(id)` with no chooser dialog. Empty catalog renders a disabled "(no workspaces)" placeholder. (c) `pickDefaultWorkspaceId(list)` (prefers `isDefault`, else first) promotes the window to a real workspace whenever the catalog hydrates without a valid active id — the legacy `__all__` sentinel still exists as a constant but is no longer reachable as the active state. Each window now owns exactly one workspace for its lifetime. Cleanup of the remaining `ALL_WORKSPACES_ID` references (filter branches in boardList / orderHelpers / hierarchy) is a follow-up.)
