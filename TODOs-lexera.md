...
- [ ] **collapseDock geometry leak** — even with the synchronous `multiview.refreshAllGeometry()` call added in 00ab49b2, the geometry push via `pushGeomDeferred` (multiviewClient.js:115) defers to the next animation frame. Result: 1-frame gap where the panel webview still paints at its pre-collapse coordinates, briefly covering the fold strip. Fix path: bypass the rAF defer for collapse operations — either call `setGeometry([…])` directly (immediate IPC, no batch) or `multiview_set_visible(label, false)` for each panel webview in the collapsing dock so it hides before the next render. Should be one small commit in workspaceShell.js's `collapseDock` body.
