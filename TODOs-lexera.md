...
- [ ] **switching boards in one view switches the other view too** — user-reported; **needs repro context**: is "view" two separate Tauri workspace windows, two split-leaf panes inside ONE window, or one shell + a hierarchy/dashboard panel that shouldn't follow? known-clean paths so far: multiview_broadcast is caller-window-scoped (d53c75bb), no cross-window storage listener responds to `lexera-last-board` (settingsStore Settings.set fires `setting:lastBoard` on the in-window LexeraRuntime bus only — no cross-window subscriber), `pollingService` only consults `lastBoard` when `!activeBoardId` (cold start). suspect remaining vectors: catalogBridge wrapping shell.openBoard and broadcasting `active-board-changed` is window-scoped via multiview_broadcast — but listener-side filter on app.js `tauriListen` is greedy `target:{kind:'Any'}` (Tauri 2 quirk pinned by the open-workspace fix) so any LEGACY board-tracking `tauriListen('active-board-changed')` would still leak across windows. need user to confirm scenario, then audit board-tracking listeners.
