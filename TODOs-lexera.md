...
- [ ] **separate `--debug` debug window with frontend tests** — user's bigger ask: "this window only opens when lexera is run with `-debug` or so". MVP shipped (LexeraDebug global + menu toggle; commit 97e7e0d7) handles the "hide overlays" requirement from inside the main window. Larger architecture pending: (a) parse `--debug` CLI flag in `lexera-kanban/src-tauri/src/main.rs`, (b) when set, create a top-level `WebviewWindow` with label `debug` hosting a new `views/debug/index.html` page, (c) move existing `views/frontendTests/` content into that window, (d) add debug controls there (hide-overlays toggle, dock snapshot inspector, ghost-detector log tail, etc.). Out of scope for the MVP but should be a 2-3-commit follow-up.
